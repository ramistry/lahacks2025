...
# Routes for the Mentor Chat System (App 2)
@app.route('/network')
def network():
...
def questionnaire():
    if request.method == 'POST':
        ...
        db.session.commit()
        return redirect('/network')  # Redirect back to the network page
...
if __name__ == '__main__':
    app.run(debug=True, port=5000)  # Port 5000 to avoid conflict
